...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true) {
  header('Location: anmeldung.php'); // Wenn der Benutzer nicht angemeldet ist, wird er zur Anmeldungsseite weitergeleitet.
  exit;
}
?>
<!DOCTYPE html>
...
<ul>
  ...
  <li><a href="http://localhost/IMS/UK295/Projekt/administrationsbereich.php">Administrationsbereich</a></li>
  <li><a href="http://localhost/IMS/UK295/Projekt/passwort.php">Passwort ändern</a></li>
...
<form action="home_admin.php" method="POST">
    ...
    <select id="kategorie" name="kategorie">
        ...
        $sql = "SELECT * FROM kategories"; // SQL-Abfrage, um alle Kategorien abzurufen.
...
if (!empty($kategorieFilter)) {
    $sql = "SELECT * FROM news WHERE kid = '$kategorieFilter' ORDER BY gueltigVon DESC"; // Wenn eine Kategorie ausgewählt wurde, werden nur die News dieser Kategorie angezeigt.
}
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($currentDate >= $gueltigVon && $currentDate <= $gueltigBis) {
            ...
            echo "<br>";
            echo "<br>";
            echo "<img src='" . $row['bild'] . "' alt='Bild' width='200'>"; // Bild anzeigen, falls vorhande
